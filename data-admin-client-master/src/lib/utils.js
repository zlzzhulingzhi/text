...
// 缩放后尺寸
export const getScaleSize = (s = 0) => {
  if (typeof(s) === 'string') {
    let unit = s.replace(/[0-9]*/g, '').replace(/\./, '')
    return `${parseFloat(s) * store.state.config.constant.scale}${unit}`
  }
  return s * store.state.config.constant.scale
}
