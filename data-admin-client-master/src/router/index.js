...
router.beforeEach(async (to, from, next) => {
  ...
  // 允许传递token登录
  if (query.token) {
    await store.commit('system/tokenUpdate', query.token)
    console.log('token', store.state.system.token)
    // await store.dispatch('system/getToken', query)

    return location.replace(url.format({
      protocol,
      host,
      search: search.replace(/token=[a-zA-Z0-9\/\.\-\_\+]+&?/, '')
    }))
  }
...
router.afterEach(async (to, from) => {
  ...
  // 定义标签页动态标题
  document.title = [to.meta.title,store.state.config.name].filter(a=>a).join(' - ')
})
