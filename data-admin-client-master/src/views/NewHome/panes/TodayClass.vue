<template>
  <ItemWrap class="padding-bottom-16" :backgroud="require('@/assets/main/bg_center_bottom.png')" :heightMiddle="true">
    <div slot="title">今年重点培训班</div>

    <div class="carousel-item title" :style="{ fontSize: $utils.getScaleSize('12px') }">
      <div class="index">序号</div>
      <div class="flex-2 text-ellipsis">班级</div>
      <div class="flex-2 text-ellipsis">所属机构</div>
      <div class="width-100 text-ellipsis">教室</div>
      <div class="width-100 text-ellipsis">日期</div>
      <div class="width-60 text-ellipsis">学员数</div>
      <div class="width-100 text-ellipsis">室内人数</div>
    </div>
    <el-carousel direction="vertical" indicator-position="none" height="360px" :interval="6000" @change="sliderChange">
      <el-carousel-item v-for="(list, index) in carouselItems" :key="index">
        <div class="flex column" v-for="(item, oIndex) in list" :key="oIndex">
          <div class="carousel-item carousel" :style="{ fontSize: $utils.getScaleSize('12px') }">
            ...
            <div class="flex-2 text-ellipsis padding-right-16">{{ item.orgName }}</div>
            <div class="width-70 text-ellipsis">{{ item.classroom }}</div>
            <div class="flex-2">
              <el-carousel height="40px" direction="vertical" :autoplay="true" indicator-position="none"
                :interval="5000">
                <el-carousel-item v-for="testItem in 3" :key="testItem">
                  <div class="carousel-item carousel">
                    <div class="width-100 text-ellipsis">{{ currentDate }}</div>
                    <div class="width-60 text-ellipsis">{{ item.studentNum }}</div>
                    <div class="width-60 text-ellipsis">{{ item.studentNum }}</div>
                  </div>
                </el-carousel-item>
              </el-carousel>
