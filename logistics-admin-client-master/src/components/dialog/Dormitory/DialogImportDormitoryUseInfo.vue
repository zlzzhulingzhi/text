<template>
  <el-dialog :visible.sync="visible" append-to-body title="导入宿舍入住信息" :close-on-click-modal="false" :before-close="close" width="800px">
    <el-card :body-style="{ padding: '16px' }">
        ...
        <div class="flex around-start width-100p step">
          ...
          <div class="flex column" v-for="index in 3" :key="index">
            ...
            <template v-if="index === 1">
              <a class="padding-left-10" :href="xlsxHref" download="导入宿舍排期表模板">
      ...
      <el-table class="margin-top-16" :data="tableData" :row-class-name="rowClassName" height="45vh" v-loading="loading.table">
        ...
        <el-table-column label="宿舍信息" min-width="200">
          <template slot-scope="{row}">
            {{row.building}}{{row.roomNo}}({{row.roomType}})
          </template>
        </el-table-column>
        <el-table-column label="使用日期" prop="useDate" min-width="200">
          <!-- <template slot-scope="{row}">
            {{new Date(row.useDate).getTime() | formatTime('yyyy-MM-DD')}}
          </template> -->
        </el-table-column>
        <el-table-column label="预订机构" prop="orgName" min-width="200"></el-table-column>
      </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      impStepEnd: false,
      impResult: {},
      xlsxHref: `${location.pathname}导入宿舍排期表模板.xlsx`
    };
  },
  ...
  methods: {
    ...
    async onImport({ file }) {
      ...
      let { code, data, msg } = await this.$api.DormitorySchedule.preImport({
        fileFormData,
      });
    ...
    async onSubmit() {
      ...
      //   })
      // );
      let result = await this.$api.DormitorySchedule.batchAdd(this.tableData);
