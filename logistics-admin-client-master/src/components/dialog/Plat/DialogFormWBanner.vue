    ...
    <template v-if="dialogType.startsWith('WBanner')">
      <el-form ref="form" :model="formData" :rules="rules" label-width="120px" size="small">
        ...
        <el-form-item prop="filePath" label="图片">
          <ImageUpload ref="ImageUpload" v-model="formData.filePath" isEdit :autoUpload="false"
                       :width="300" :height="59.375" :options="{
                         fixedNumber: [1920, 380],
                         autoCropWidth: 1920,
                         autoCropHeight: 1080,
                       }">
...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      this.formData.filePath = await this.$refs.ImageUpload.uploadFile()
