<template>
  <TableView :options="options" :params.sync="filterData">
    <template v-slot:main>
      ...
      <el-select
        v-model="choseApplyStatus"
        value-key="id"
        size="small"
        placeholder="申请状态"
        clearable
        class="margin-bottom-8 margin-right-12 width-140"
        style="order: 120">
        <el-option v-for="(item, index) in approvalRecordsStatus" :key="index" :label="item.name" :value="item">
        </el-option>
      </el-select>
    </template>
    ...
    <el-table class="margin-top-16" :data="tableData" :height="$utils.FullViewHeight(226)" v-loading="loading.table">
      ...
      <el-table-column label="状态" prop="status" width="100">
        <template slot-scope="{ row }">
          <span v-if="row.flowStatus && row.applyStatus">
            <i class="dot" :class="`bg-${approvalRecordsStatus.find(item=>item.flowStatus===row.flowStatus&&item.applyStatus===row.applyStatus).status}`"></i>
            <span class="text-6">{{ approvalRecordsStatus.find(item=>item.flowStatus===row.flowStatus&&item.applyStatus===row.applyStatus).name }}</span>
          </span>
        </template>
      ...
      <el-table-column label="操作" width="120" fixed="right">
        <template slot-scope="{ row }">
          ...
          <el-button
            type="text"
            size="small"
            @click.stop="onDropRecord(row)"
            v-if="row.flowStatus === 2 && row.applyStatus === 1"
            >作废</el-button
          >
          <el-button type="text" size="small" @click.stop="onApproveDetail(row)">审批情况</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
    ...
    return {
      options: {
        ...
        main: {
          ...
          // status: {
          //   label: '审批状态',
          //   options: 'approvalRecordsStatus',
          //   value: 11,
          // },
        },
      },
      // 申请状态
      choseApplyStatus: { id:2, flowStatus: 2, applyStatus: 1, name: '审批通过', status: 'success' },
      ...
      approvalRecordsStatus: [
        { id:1, flowStatus: 1, applyStatus: 1, name: '审批中', status: 'warning' },
        { id:2, flowStatus: 2, applyStatus: 1, name: '审批通过', status: 'success' },
        { id:3, flowStatus: 4, applyStatus: 1, name: '审批驳回', status: 'emb' },
        { id:4, flowStatus: 2, applyStatus: 2, name: '已作废', status: '9' },
      ],
    };
  },
  computed: {
    ...
    params() {
      let {flowStatus, applyStatus} = this.choseApplyStatus
  ...
  methods: {
    ...
    async getTableData() {
      ...
      let { code, data } = await this.$api.Approve.pageRecords(this.params);
    ...
    // 操作 - 作废
    async onDropRecord(param) {
      await this.$confirm(`确认作废“${param.courseName}”吗？`, {
        title: '作废',
        type: 'warning'
      })
      let {code} = await this.$api.Approve.recordInvalidate({
        id: param.id,
        applyStatus: 2
      })
      if(code !== 200) return false
      this.$msg(`"${param.courseName}"作废`)
      this.getTableData()
    }
  },
};
