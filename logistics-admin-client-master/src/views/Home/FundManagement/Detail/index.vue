...
<div>
    ...
    <el-tabs v-model="activePageName" class="view" v-loading="loading.page">
      <el-tab-pane v-for="item in pages" :key="item.id" :name="item.id" :label="item.name">
        <component type="View" :is="item.id" :info.async="info" @confirm="onConfirm"></component>
      </el-tab-pane>
    </el-tabs>

    <!-- <el-button type="primary" size="small" class="absolute goBackButton" @click="$router.go(-1)">返回</el-button> -->
...
export default {
  ...
  methods: {
    ...
    async getDetail () {
      ...
      let { code, data } = await this.$api.Approve.qualificationdDetail({
        id: this.id
      })
