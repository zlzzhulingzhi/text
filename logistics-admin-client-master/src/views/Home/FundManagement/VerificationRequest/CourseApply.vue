<template>
  <TableView :options="options" :params.sync="filterData">
    ...
    <el-table class="margin-top-16" :data="tableData" :height="$utils.FullViewHeight(237)" v-loading="loading.table">
      ...
      <el-table-column label="项目名称(主题名称)" prop="projectName" min-width="150"></el-table-column>
...
export default {
  ...
  methods: {
    async getTableData() {
      ...
      let { code, data } = await this.$api.Approve.allowancePage(this.params)
    ...
    async onDetail(row) {
      ...
      //   }
      // })
     this.$utils.WindowOpenInParentFrame(`/FundManagement/CourseDetail?id=${row.id}`)
    },
    // 操作 - 审批情况
    onApproveDetail(row) {
      this.$refs.DialogApplyForFunding.open({
