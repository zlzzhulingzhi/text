  ...
  <el-container class="height-100p">
    ...
    <TableView class="flex-1 overflow-auto" :options="optionsComputed" :params.sync="filterData">
      <template v-slot:main>
        ...
        <div class="margin-bottom-8 margin-right-12 flex">
      ...
      <el-table class="margin-top-16" :data="tableData"
        ...
        <el-table-column label="角色" prop="roleNames" min-width="100">
          <template slot-scope="{row}">
            {{ row.roleNames || '-' }}
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      this.loading.table = true
      let {code, data} = await this.$api.LogisticsEmployee.pageAdmin(this.params)
      this.loading.table = false
      if (code !== 200) return false
    ...
    async onDelete({id, realName}) {
      ...
      this.loading.table = true
      let {code} = await this.$api.LogisticsEmployee.delete({idList: [id]})
      this.loading.table = false
      if (code !== 200) return false
    ...
    async onBatchDelete() {
      ...
      this.loading.table = true
      let {code} = await this.$api.LogisticsEmployee.delete({idList: this.selectionList})
      this.loading.table = false
      if (code !== 200) return false
    ...
    async onBatchEnabled(flag, idList, name) {
      this.loading.table = true
      let {code} = await this.$api.LogisticsEmployee.batchAdminEnabled({
        idList,
        flag,
