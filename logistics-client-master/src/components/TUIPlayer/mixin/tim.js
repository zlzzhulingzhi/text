...
import {decodeText} from '@/components/TUIPlayer/utils/utils'
...
export default {
  ...
  methods: {
    ...
    // 初始化，创建tim实例
    async initTim() {
      console.log('%c 初始化tim实例','color:#333;background: #F9C')
      this.tim = TIM.create({
    ...
    async loginTim() {
      console.log(`%c 登录tim ${this.userInfo.userId}`,'color:#333;background: #F9C')
      try {
    ...
    // 加入创建的直播群
    async joinGroup() {
      console.log('%c 加入直播群','color:#333;background: #F9C')
      try {
    ...
    // 判断群组是否存在
    async searchGroup() {
      console.log(`%c 搜索直播群 ${this.groupID}`,'color:#333;background: #F9C')
      try {
        // 存在的情况
    ...
    async onReadyStateUpdate({name}) {
      console.log(`%c onReadyStateUpdate ${name} sdk完毕`,'color:#333;background: #F9C')
