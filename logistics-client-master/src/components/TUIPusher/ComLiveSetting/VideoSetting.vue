...
<template>
  <div class="setting-item">
    <div class="select-item">
      <span class="title">{{ $t('device.Camera') }}</span>
      <el-select class="select-style" v-model="choseCameraId" :placeholder="$t('device.Select a camera')">
        <el-option v-for="item in cameraList" :key="item.deviceId" :label="item.label"
                   :value="item.deviceId"></el-option>
      </el-select>
    </div>

    <div id="preview-stream" class="local-stream-preview" :class="rotateClass"
         v-show="choseCameraId !== undefined"></div>
    <div class="local-stream-preview flex center-center text-f" v-show="choseCameraId === undefined">请检查摄像头是否安装</div>

    <el-checkbox class="mirror-checkbox" v-model="isSetMirror" :disabled="liveStage !== LIVE_STAGE.NOT_STARTED">
      {{ $t('Mirror') }}
    </el-checkbox>

    <div class="select-item">
      <span class="title">{{ $t('Resolution') }}</span>
      <el-select class="select-style" v-model="profileValue" :placeholder="$t('Select a resolution')">
        <el-option v-for="(item, index) in profileList" :key="index" :label="item" :value="item"></el-option>
      </el-select>
    </div>

    <div class="select-item">
      <span class="title">{{ $t('Frame Rate(fps)') }}</span>
      <el-select class="select-style" v-model="videoSetting.frameRate" :placeholder="$t('Select a frame rate')">
        <el-option v-for="(item, index) in [15, 30]" :key="index" :label="item" :value="item"></el-option>
      </el-select>
    </div>

    <div class="select-item">
      <span class="title">{{ $t('Bitrate(kbps)') }}</span>
      <el-input-number class="select-style" v-model="videoSetting.bitrate" controls-position="right" :min="1"
                       :max="9000" :step="100"></el-input-number>
    </div>
  </div>
</template>
  ...
  watch: {
    ...
    videoSetting: {
      ...
      handler(val) {
        ...
        this.$store.commit('livePusher/updateVideoProfile', val)
      }
    },
    // 关闭弹窗时销毁视频流，释放设备
    dialogVisible(val) {
    ...
    activeTab: {
      ...
      async handler(val, oldVal) {
        ...
        if (val === 'video') {
          if (this.choseCameraId === undefined) return false
    ...
    choseCameraId(val) {
      ...
      this.$store.commit('livePusher/updateActiveCamera', choseCameraDevice)
    },
    isSetMirror(val) {
      this.$store.commit('livePusher/updateIsSetMirror', val)
    },
    profileValue(val) {
