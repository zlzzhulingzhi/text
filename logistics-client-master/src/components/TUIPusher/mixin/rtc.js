...
let logStyle = 'background: #1BCEC8;color:#103c99;line-height:24px'
...
export default {
  ...
  methods: {
    ...
    async initBeauty() {
      ...
      if (!this.localStream.getVideoTrack()) return false
    ...
    async handleLeave() {
      ...
      try {
        ...
        this.isLeaving = false
        console.log('%c 退房失败', 'background: #8F3;color:#985', error)
    ...
    startGetAudioLevel() {
      this.audioLevelInterval = setInterval(() => {
        ...
        if (level >= 0.02) {
          ...
          this.$store.commit('livePusher/updateAudioLevel', level)
        } else {
          this.$store.commit('livePusher/updateAudioLevel', 0)
        }
      }, 200)
    },
