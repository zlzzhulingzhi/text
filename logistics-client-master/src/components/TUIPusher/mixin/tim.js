...
import {decodeText} from '@/components/TUIPusher/utils/utils'
...
export default {
  ...
  methods: {
    ...mapMutations({
      updateGroupInfo:'livePusher/updateGroupInfo'
    }),
    // ------------ 初始化及销毁相关 ----------
    // 初始化，创建tim实例
    async initTim() {
    ...
    // 登出im
    async logout() {
      console.log(`%c 登出im`, 'background:#572;color:#fff')
      try {
    ...
    // 加入创建的直播群
    async joinGroup() {
      console.log(`%c 加入直播群 ${this.groupID}`, 'background: #572;color:#fff')
      try {
    ...
    // 创建群组
    async createGroup() {
      console.log(`%c 创建直播群 ${this.groupID}`, 'background:#572;color:#fff')
      try {
    ...
    async dismissGroup(groupID) {
      console.log(`%c 解散直播群 ${groupID}`, 'background: #572;color:#fff')
      try {
        await this.tim.dismissGroup(groupID)
      } catch (imError) {
        // this.$message.error(i18n.t('tim.Failed to dismiss streaming group'))
        console.error('%c dismissGroup error:', 'background: #572;color:#fff', imError) // 解散群组失败的相关信息
      }
    },
    // 判断群组是否存在
    async searchGroup() {
    ...
    async getGroupProfile() {
      ...
      console.log(`%c 获取在线人数 ${data.group.memberCount}`, 'background:#572;color:#fff',data)
      this.updateGroupInfo(data.group)
    },
    // 监听事件
    handleTimEvents() {
    ...
    handleTimEventsOff() {
      ...
      this.tim.off(TIM.EVENT.SDK_NOT_READY, this.onNotReadyStateUpdate)
      this.tim.off(TIM.EVENT.KICKED_OUT, this.onKickOut)
      this.tim.off(TIM.EVENT.ERROR, this.onError)
      this.tim.off(TIM.EVENT.MESSAGE_RECEIVED, this.onTextMessageReceived)
    },
    ...
    async handleJoinGroupTip(message) {
      console.log(`%c 监听到加群消息 ${message.nick}`, 'color:#333;background: #F9C', message)
    ...
    async handleQuitGroupTip(message) {
      console.log(`%c 监听到退群消息 ${message.nick}`, 'color:#333;background: #F9C', message)
