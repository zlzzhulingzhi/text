<template>
  <el-dialog :visible.sync="visible" append-to-body title="教室详情" :close-on-click-modal="false" :before-close="close" width="824px">
    ...
    <Banner :list="classroomPicture" errorInfo="教室图片" v-if="classroomPicture&&classroomPicture.length"></Banner>
...
export default {
  ...
  methods: {
    ...
    async getClassroomDetail () {
      let {code, data} = await this.$api.Classroom.manageDetail({
        id: this.classroomId
      })
