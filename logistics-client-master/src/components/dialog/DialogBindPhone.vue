<template>
  <el-dialog :visible.sync="visible" append-to-body :title="dialogInfo.title" :close-on-click-modal="false"
    ...
    <el-form ref="form" :model="formData" :rules="rules" label-position="top" hide-required-asterisk>
      ...
      <el-form-item>
        <el-button class="width-100p margin-top-16" type="primary" @click="onSubmit" :loading="loading.submit">绑定
        </el-button>
      </el-form-item>
    </el-form>
  </el-dialog>
...
import {SmsTimeStampBind} from '@/lib/storage'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      smsTimeStamp: SmsTimeStampBind.get(),
      timeStamp: Date.now(),
      loading: {
        send: false,
        submit: false
  ...
  methods: {
    ...
    async onSendSms() {
      ...
      if (code !== 200) return false
      SmsTimeStampBind.set(this.smsTimeStamp = Date.now())
      this.tick()
    },
    tick() {
      let fn = () => {
