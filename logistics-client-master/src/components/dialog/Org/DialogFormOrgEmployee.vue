...
export default {
  ...
  data() {
    ...
    return {
      ...
      typeMapping: {
        OrgEmployeeCreate: {title: '新增员工', submitFn: this.$api.OrgEmployee.add, showPassword: true, type: 'Create'},
        OrgEmployeeEdit: {title: '编辑员工', submitFn: this.$api.OrgEmployee.update, showPassword: false, type: 'Edit'},
        OrgEmployeeEditRole: {title: '员工授权', submitFn: this.$api.OrgEmployee.updateRole, showPassword: false, type: 'Edit'}
      },
      defaultFormData,
      formData: {
        ...defaultFormData
  ...
  methods: {
    ...
    async open(data = {}) {
      ...
      if (/(Edit|EditRole)$/.test(this.dialogType)) {
        let {code, data: d} = await this.$api.OrgEmployee.detail({
          id: data.formData.id
        })
