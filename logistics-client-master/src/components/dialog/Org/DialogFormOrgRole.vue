      ...
      <el-form ref="form" :model="formData" :rules="rules" label-width="120px" size="small">
        ...
        <el-form-item label="功能权限">
          ...
          <PermissionTree type="OrgRole" v-model="formData.menuIdList"
                          class="tree-wrapper overflow-auto" v-if="visible"></PermissionTree>
        </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      typeMapping: {
        OrgRoleCreate: {title: '新增角色', submitFn: this.$api.OrgRole.add, type: 'Create'},
        OrgRoleEdit: {title: '编辑角色', submitFn: this.$api.OrgRole.update, type: 'Edit'}
      },
      defaultFormData,
      formData: {
        ...defaultFormData
  ...
  methods: {
    ...
    async open(data = {}) {
      ...
      if (this.dialogType === 'OrgRoleEdit') {
        let {code, data: d} = await this.$api.OrgRole.detail({
          id: data.formData.id
        })
