...
// 格式化时间
// unit 单位 ，默认 毫秒 ms
// format 输出格式: 示例 DD天hh时mm分ss秒
export const DurationToCn = (value, options = {}) => {
  let format = options.format || 'DD天'
  let unit = options.unit || 'ms'
  ...
  let _ms = value * unitMaps[unit]

  let DD = Math.floor(_ms / unitMaps.d)
...
export const getElementGlobalPosition = function (el) {
  let x = el.offsetLeft
  let y = el.offsetTop
...
export const getDate = function (arg = 0) {
  // 将 Date对象转换成 yyyy-MM-dd格式
  let DateStr = date => [
    date.getFullYear(),
    date.getMonth() + 1,
    date.getDate()].map(n => n < 10 ? '0' + n : n).join('-');
  // 将 数字、Date对象 统一为 Date对象
  let DateObj = param => {
  ...
  // 返回 Date字符串 或 字符串数组
  if (arguments.length === 1) return DateStr(DateObj(arg));
  return Array.prototype.map.call(arguments, arg => DateStr(DateObj(arg)));
};

export const WindowOpenInParentFrame = (routePath) => {
  let {href,origin} = window.parent.location
  let hash = href.match(/\/Embed\/System:\w+/)[0]
  window.open(`${origin}/#${hash}?routePath=${routePath}`, `${Date.now()}_blank`)
}
