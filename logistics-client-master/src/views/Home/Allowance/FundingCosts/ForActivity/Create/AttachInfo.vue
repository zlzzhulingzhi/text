  ...
  methods: {
    ...
    async onInitFormData() {
      if (this.info && this.info.id) {
        ...
        let {code, data} = await this.$api.ApplyCost.attachListActivity({
          id: this.info.id
        })
    ...
    async onSaveImage(image) {
      ...
      let {code} = await this.$api.ApplyCost.attachSaveActivity({
    ...
    async onDeleteImage(row, item, index) {
      let {code} = await this.$api.ApplyCost.attachRemoveActivity({
        id: item.id
      })
