...
<template>
  <el-card>
    ...
    <el-table class="margin-bottom-16" :data="tableData" :height="pageInfo.tableHeight" v-loading="loading.page">
      ...
      <el-table-column label="操作" width="88" v-if="!detailCSS">
        <template slot-scope="{row}">
...
export default {
  ...
  methods: {
    ...
    async onInitFormData() {
      if (this.info && this.info?.applyAllowanceDetailResponse?.id) {
        // 详情页 修改 detailCSS
        if(this.type === 'View') this.detailCSS = true

        this.loading.page = true
        let {code, data} = await this.$api.ApplyCost.attachListCourse({
          id: this.info.applyAllowanceDetailResponse.applyId
        })
    ...
    async onSaveImage(image) {
      ...
      let {code} = await this.$api.ApplyCost.attachSaveCourse({
        applyId: this.info.applyAllowanceDetailResponse.applyId,
    ...
    async onDeleteImage(row, item, index) {
      let {code} = await this.$api.ApplyCost.attachRemoveCourse({
        id: item.id
      })
