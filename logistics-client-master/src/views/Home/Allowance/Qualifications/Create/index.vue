...
<div>
    ...
    <el-tabs v-model="activePageName" :class="pageInfo.pageClassName" v-loading="loading.page">
      <el-tab-pane v-for="item in pages" :key="item.id" :name="item.id" :label="item.name" :disabled="!id">
        <component :type="type" :is="item.id" :info="info" @confirm="onConfirm"></component>
      </el-tab-pane>
    </el-tabs>
...
<script>
import BaseInfo from '@/views/Home/Allowance/Qualifications/Create/BaseInfo'
import AttachInfo from '@/views/Home/Allowance/Qualifications/Create/AttachInfo'
    ...
    pageInfo() {
      let typeMapping = {
        Create: {
          ...
          detailAPI: this.$api.ApplyQualification.detail,
          submitAPI: this.$api.ApplyQualification.save
        },
        Edit: {
          pageClassName: 'edit',
          detailAPI: this.$api.ApplyQualification.detail,
          submitAPI: this.$api.ApplyQualification.update
        },
        View: {
          pageClassName: 'view',
          detailAPI: this.$api.ApplyQualification.detail,
