<template>
	<view class="uni-date">
      ...
      :default-value="defaultValue"
			...
			:hideSecond="hideSecond" @confirm="mobileChange" @maskClose="close" @touchCloseOrMakeSureButton="transmitIfClose" />
	</view>
</template>
<script>
	/**
  ...
  import { getDateTime, getDate, getTime, getDefaultSecond, dateCompare, checkDate, fixIosDateFormat } from './util'
	...
	export default {
      ...
      this.platform()
		...
		methods: {
      transmitIfClose() {
        this.$emit('touchDataPicker', false)
      },
      initI18nT() {
        ...
        if(typeof navigator !== "undefined"){
          ...
          return
			...
			show() {
        this.$emit('touchDataPicker', true)
				if (this.disabled) {
					return
				}
				this.platform()
