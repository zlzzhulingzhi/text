  ...
  <view>
    ...
    <view>
      ...
      <template v-if="listPage.length">
        ...
        <view v-if="listPage.length === total" class="px-4 py-5">
          <text>{{ `${tabList.find(v => v.id === currentTab).name}总数：${total}` }}</text>
        </view>
      </template>
    ...
    <u-picker :show="show.roomType" :columns="[dormitoryList]" keyName="name" @cancel="cancelType"
...
export default {
  ...
  computed: {
    ...
    tabList() {
      return [
        { id: 0, name: '未入住', submitFn: this.$api.Logistics.dormitoryPage },
        { id: 1, name: '已入住', disabled: !this.isLogin, submitFn: this.$api.Logistics.dormitoryPage }
      ]
    },
  ...
  methods: {
    async getDetail() {
      ...
      // console.log('this.params2222 === ', this.params)
      this.params.useDate = this.$u.timeFormat(this.form.useDate, 'yyyy-mm-dd');
      let { code, data } = await this.$api.Logistics.dormitoryPage(this.params)
      uni.hideLoading()
    ...
    async collapseChange(item) {
      ...
      let { code, data } = await this.$api.Logistics.dormitorySchedule({
    ...
    tabClick(item) {
      ...
      this.clearData()
      this.params.checkIn = this.currentTab;
      this.getDetail()
    }
  }
}
