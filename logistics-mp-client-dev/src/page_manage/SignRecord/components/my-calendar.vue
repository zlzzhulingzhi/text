<template>
    <view class="calendar-wrapper mx-2 rounded-lg box-border">
        ...
        <view :class="{ hide: !monthOpen }" class="content" :style="{ height: height }">
            <view :style="{ top: positionTop + 'rpx' }" class="days">
                <view class="item" v-for="(item, index) in dates" :key="index">
                    ...
                    <view v-if="isMarkDay(item.year, item.month, item.date)">
                        <view class="workDay" v-if="isNormalDay(item.year,item.month,item.date)"></view>
                        <view class="notSigned" v-else></view>
                    </view>
    ...
    methods: {
        ...
        isNormalDay(y, m, d) {
            ...
            for (let i = 0; i < this.markDays.length; i++) {
                let dy = `${y}-${m}-${d}`;
                if (this.markDays[i].clockInDate == dy && this.markDays[i].clockIned) {
                    flag = true;
                    break;
                }
            }
        ...
        isMarkDay(y, m, d) {
            ...
            for (let i = 0; i < this.markDays.length; i++) {
                if (this.markDays[i].clockInDate == date) {
                    flag = true;
                    break;
                }
            }
        ...
        changeMonth(type){
            ...
            this.$emit('onYearChange', {year:this.y,month:this.m});
            this.dates = this.monthDay(this.y, this.m);
        }
    }
};
