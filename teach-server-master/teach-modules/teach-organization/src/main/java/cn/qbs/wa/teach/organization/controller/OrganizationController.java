...
public class OrganizationController {
    ...
    //@RequiresPermissions("application:list")
    @Log(title = "机构部门列表显示", businessType = BusinessType.OTHER)
    ...
    @Log(title = "机构部门树形列表", businessType = BusinessType.OTHER)
    ...
    @PostMapping("orgList")
    @ApiOperation("学员机构列表")
    public R<List<OrganizationListResponse>> orgList() {
        List<Organization> organizations=this.organizationService.lambdaQuery().eq(Organization::getEnabled,1).list();
        List<OrganizationListResponse> organizationListResponses=BeanUtil.copyToList(organizations, OrganizationListResponse.class);
        for (OrganizationListResponse organizationListResponse:organizationListResponses) {
            organizationListResponse.setOrgId(organizationListResponse.getId());
        }
        return R.ok(organizationListResponses);
    }

    @PostMapping("lite/orgList")
    @ApiOperation("小程序机构列表")
    public R<List<Organization>> liteOrgList() {
        List<Organization> organizations=this.organizationService.lambdaQuery().eq(Organization::getEnabled,1).list();
