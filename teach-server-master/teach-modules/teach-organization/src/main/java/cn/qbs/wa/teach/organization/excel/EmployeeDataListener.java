 ...
 * @author Jiaju Zhuang
 */
// 有个很重要的点 EmployeeDataListener 不能被spring管理，要每次读取excel都要new,然后里面用到spring可以构造方法传进去
@Slf4j
public class EmployeeDataListener implements ReadListener<EmployeeData> {
     ...
     * @param deptId
     */
    public EmployeeDataListener(ImportEmployeeService importEmployeeService, Long orgId, String eventId, Long deptId) {
    ...
    private void saveData() {
        if(CollectionUtils.isNotEmpty(cachedDataList)){
            ...
            importEmployeeService.saveImportData(cachedDataList,orgId,eventId,deptId);
            log.info("存储数据库成功！");
        }
