...
public class UnitController {
     ...
     */
    @PostMapping("list")
    @ApiOperation("单位职员查询单位列表")
    public R<List<UnitResponse>> listUnit(@RequestBody UnitPageRequest params) {
        if(StringUtils.isNotBlank(params.getName())){
            List<Unit> unitList=this.unitService.lambdaQuery().like(Unit::getName,params.getName()).list();
            List<UnitResponse> appUnitResponseList=TreeUtil.copyBeanList(unitList, UnitResponse.class);
            return R.ok(appUnitResponseList);
        }
        List<Unit> unitList=this.unitService.lambdaQuery().list();
        List<UnitResponse> appUnitResponseList=TreeUtil.copyBeanList(unitList, UnitResponse.class);
        return R.ok(appUnitResponseList);
    }
