...
import cn.qbs.wa.train.basic.mapper.DictMapper;
...
public class DictServiceImpl extends ServiceImpl<DictMapper, Dict> implements DictService {
    ...
    public boolean add(DictAddRequest params) {
        ...
        if (params.getParentId() == null) {
            List<Dict> existList =
                    list(new LambdaQueryWrapper<Dict>().eq(Dict::getParentId, Constants.DB_FAIL).eq(Dict::getCode,
                            params.getCode()));
    ...
    @Override
    public IPage<DictPageResponse> pageByCode(DictPageRequest params) {
        return baseMapper.pageBycode(params.createMpPage(), params);
    }
    ...
    public boolean update(DictUpdateRequest params) {
        ...
        List<Dict> existList =
                list(new LambdaQueryWrapper<Dict>().eq(Dict::getDictKey, params.getDictKey()).eq(Dict::getCode,
                        params.getCode()).ne(Dict::getId, params.getId()));
        if (CollectionUtils.isNotEmpty(existList)) {
            throw new ServiceException("已存在重复的key");
        }
        Dict dict = new Dict();
    ...
    public List<TreeDictResponse> treeDict(TreeDictRequest params) {
        List<Dict> dictList =
                list(new LambdaQueryWrapper<Dict>().eq(Dict::getCode, params.getCode()).orderByAsc(Dict::getSort));
    ...
    public List<DictListResponse> childList(DictChildRequest params) {
        List<Dict> dictList =
                list(new LambdaQueryWrapper<Dict>().eq(Dict::getParentId, params.getParentId()).orderByAsc(Dict::getSort));
        return BeanUtil.copyToList(dictList, DictListResponse.class);
    }

    @Override
    public List<DictListResponse> childLists(DictChildRequest params) {
        List<Dict> dictList = list(new LambdaQueryWrapper<Dict>().eq(Dict::getCode, params.getCode())
                .ne(Dict::getParentId, params.getParentId()).orderByAsc(Dict::getSort));
        return BeanUtil.copyToList(dictList, DictListResponse.class);
    }
