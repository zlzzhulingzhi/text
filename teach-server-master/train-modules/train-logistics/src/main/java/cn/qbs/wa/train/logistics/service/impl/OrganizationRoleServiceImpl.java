...
import cn.qbs.wa.train.logistics.mapper.EmployeeRoleMapper;
...
public class OrganizationRoleServiceImpl extends ServiceImpl<OrganizationRoleMapper, OrganizationRole> implements OrganizationRoleService {
    ...
    public boolean add(OrganizationRoleAddRequest params) {
        ...
        saveOrganizationRoleMenu(params.getMenuIdList(), organizationRole.getId(),organizationRole.getOrgId());
        return flag;
    }

    private void saveOrganizationRoleMenu(List<Long> menuIdList, Long roleId,Long orgId) {
    ...
    private void getAndSetMenuNames(List<OrganizationRolePageResponse> records) {
        if (CollectionUtils.isNotEmpty(records)) {
            ...
            if (CollectionUtils.isNotEmpty(menuIdList)) {
                R<List<MenuListResultDTO>> menuResultList = remoteMenuService.listAllMenu();
    ...
    private void setMenuName(OrganizationRolePageResponse response, List<MenuListResultDTO> menuListResultDTOList) {
        ...
        // 机构管理员拥有所有菜单权限
        if (InitRoleEnum.ORG_MASTER.getCode().equals(response.getCode())) {
            response.setMenuNames(menuListResultDTOList.stream().map(MenuListResultDTO::getName).collect(Collectors.joining(",")));
        }
    }
    ...
    public boolean update(OrganizationRoleUpdateRequest params) {
        ...
        saveOrganizationRoleMenu(params.getMenuIdList(), params.getId(),organizationRole.getOrgId());
